// The per-account S3 bucket that holds the Lambdas' code zip.
//
// CloudFormation can't carry a Lambda zip inline (the template is passed as TemplateBody), so
// the code must live in S3 and the template references it by bucket+key. This bucket is one of
// the TWO things AffiliatePoppy creates outside its stack (the other is the pair of SSM
// parameters holding the merchant's Stripe secrets) — so, like them, (a) it carries the
// attribution tags and (b) the teardown hook empties and deletes it, because the stack delete
// won't (AGENTS.md §4).
//
 ...
 *  (affiliatepoppy-deploy-*), lowercased and DNS-safe. */
export function deployBucketName(accountId: string, region: string): string {
  return `affiliatepoppy-deploy-${accountId}-${region}`.toLowerCase();
}
