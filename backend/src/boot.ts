...
export function brokerCredentialsProvider(
  ...
  const post = async (body?: unknown): Promise<Response> => {
    ...
    try {
      ...
      throw new Error(
        `AffiliatePoppy is waiting for AWS access from AgentsPoppy. If this doesn't clear, reopen AgentsPoppy and check AffiliatePoppy is connected. (${(e as Error).message})`,
      );
    }
  };
  ...
  async function mint(): Promise<AwsCredentialIdentity> {
    ...
    for (;;) {
      if (!res.ok) {
        let message = `AgentsPoppy won't grant AWS access right now (${res.status}). Check that AffiliatePoppy is connected and your AWS access is healthy in AgentsPoppy.`;
        try {
