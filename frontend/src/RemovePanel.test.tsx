...
describe("removing AffiliatePoppy takes a deliberate confirmation", () => {
  ...
  it("names the blast radius rather than asking a bare 'are you sure?'", async () => {
    ...
    // What actually disappears: the people, their codes, and the record of what they earned.
    expect(dialog).toHaveTextContent(/every commission recorded for them/i);
    expect(dialog).toHaveTextContent(/can't be undone/i);
    // …and the way out that exists BEFORE they press it.
    expect(dialog).toHaveTextContent(/export it from the Ledger tab/i);
  });
  ...
  it("keeps the destroy button disarmed until the name is typed exactly", async () => {
    ...
    await userEvent.type(screen.getByRole("textbox"), "AffiliatePoppy");
  ...
  it("makes the type-to-enable requirement visible, with a live hint while typing", async () => {
    ...
    expect(screen.getByPlaceholderText("AffiliatePoppy")).toBeInTheDocument();
